...
async def create_pet_profile(pet_profile: UserPetProfileCreate, current_user: dict = Depends(get_current_user)):
    ...
    if not ObjectId.is_valid(pet_profile.pet_type_id):
        raise HTTPException(status_code=400, detail="Invalid pet type ID")
    
    pet_type = await db.pets.find_one({"_id": ObjectId(pet_profile.pet_type_id)})
    if not pet_type:
...
@router.get("/", response_model=List[UserPetProfileResponse])
async def get_my_pet_profiles(current_user: dict = Depends(get_current_user)):
    db = await get_database()
    user_id = str(current_user["_id"])
